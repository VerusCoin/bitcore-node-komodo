...
Bitcoin.prototype.formatCurrencies = function(addressArg, callback) {
  var self = this;
  var results = {};

  var keys = Object.keys(addressArg);
  console.log(keys);
  async.each(keys, function(key, cb) {
    self.client.getCurrency(key, function(err, response) {
      if (err) {
        return cb(self._wrapRPCError(err));
      }
      if(addressArg[key] > 0)
        results[response.result.fullyqualifiedname] = addressArg[key];
      cb();
    });
  }, function(err) {
    if (err) {
      ...
      function getBalance(done) {
        self.getAddressBalance(addresses, options, function(err, data) {
          if (err) {
            ...
            self.formatCurrencies(summary.currencybalance, function(err, data) {
              if (data) {
                summary.currencybalances = data;
              }
              done();
            }) 
        });
