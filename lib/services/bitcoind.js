...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  if (balance) {
    ...
    this.client.getAddressBalance({addresses: addresses, friendlynames: true}, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      var result = response.result;
      
      // Transform currencybalance to use currency names instead of i-addresses
      if (result.currencybalance && result.currencynames) {
        var transformedBalance = {};
        for (var iAddress in result.currencybalance) {
          if (result.currencynames[iAddress]) {
            transformedBalance[result.currencynames[iAddress]] = result.currencybalance[iAddress];
          } else {
            transformedBalance[iAddress] = result.currencybalance[iAddress];
          }
        }
        result.currencybalance = transformedBalance;
      }
      
      self.balanceCache.set(cacheKey, result);
      ...
      function getBalance(done) {
        self.getAddressBalance(addresses, options, function(err, data) {
          if (err) {
            ...
            summary.currencybalances = data.currencybalance;
            done();
        });
      },
      function getIdentity(done) {
        if(addresses[0].startsWith('i') || addresses[0].endsWith('@') ){
      
          self.getIdentity(addresses, function(err, data) {
            if (err) {
              return done(err);
            }
            summary.identityinfo = data;
...
Bitcoin.prototype.getBlockHash = function(blockArg, callback) {
  ...
  if (_.isNumber(blockArg) || (blockArg.length < 40 && /^[0-9]+$/.test(blockArg))) {
    ...
    callback(null, "Not a valid height");
  }
};
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        self.transactionCache.set(txid, response.result);
        done(null, response.result);
      });
    }, callback);
  }
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      var ident = null;
      if('identitycommitment' in out.scriptPubKey)
        ident = {'identitycommitment': true};
      tx.outputs.push({
        ...
        reserveoutput: out.scriptPubKey.reserveoutput,
        script_reserve_balance: out.scriptPubKey.reserve_balance,
        identityprimary: out.scriptPubKey.identityprimary,
        reserve_balance: out.reserve_balance,
        script_reserve_balance: out.scriptPubKey.reserve_balance,
        currencydefinition: out.scriptPubKey.currencydefinition,
        finalizeNotarization: out.scriptPubKey.finalizeNotarization,
        reservetransfer: out.scriptPubKey.reservetransfer,
        crosschainimport: out.scriptPubKey.crosschainimport,
        crosschainexport: out.scriptPubKey.crosschainexport,
        identitycommitment: ident,
        pbaasNotarization: out.scriptPubKey.pbaasNotarization
      });
    }
  }
 ...
 */
 Bitcoin.prototype.getBlockCount = function(callback) {
  var self = this;
  this.client.getBlockCount(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
Bitcoin.prototype.coinSupply = function(callback) {
  var self = this;
...
/**
 * Will return the identity information, the output will have the format:
  { "identity": {
    "version": 2,
    "flags": 1,
    "primaryaddresses": [
      "RChYoFDhk8wHhoitj59CS2xPhBzrV4mt19"
    ],
    "minimumsignatures": 1,
    ...
    "name": "monkins",
    "contentmap": {
      "0000000053444920736574616572432073757265": "4455468652054696d65732030332f4a616e2f32303039204368616e63656c6c6",
      "0100000053444920736574616572432073757265": "f72206f6e206272696e6b206f66207365636f6e64206261696c6f757420666f7",
      "0200000053444920736574616572432073757265": "00000000000000000000000000000000000000000000000000022062616e6b73",
      "ffff00ff0000ff00ff0000ff00ff0000ffffffff": "ffff00ff0000ff00ff0000ff00ff0000ff00ff0000ff00ff0000ff00ff00ffff"
    },
  ...
  "cansignfor": true,
  "blockheight": 245576,
  "txid": "bc68dd6d854a7f5383f494ebd95c3fd0f2563197b88617240ccbc1e1aa1f94e7",
  "vout": 1
 * }
 * @param {String} identity - A VRSC Identity or i-address
 * @param {Function} callback
 */
 Bitcoin.prototype.getIdentity = function(identity, callback) {
  var self = this;

    self._tryAllClients(function(client, done) {
      client.getIdentity(identity, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        var result = response.result;
